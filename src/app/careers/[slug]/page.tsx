...
export default async function JobPage({ params }: { params: Promise<Params> }) {
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 pt-32 pb-24 sm:px-6">
      ...
      <h1 className="font-display mt-4 text-5xl font-semibold">{job.title}</h1>
      <p className="mt-5 text-lg text-muted">{job.summary}</p>
      <h2 className="font-display mt-10 text-2xl">What you will do</h2>
      ...
      <Link href="/contact" className="mt-8 inline-block rounded-full bg-gold px-6 py-3 font-semibold text-white">
        Contact the studio
      </Link>
    </section>
  );
